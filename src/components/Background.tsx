    ...
    <div className="fixed inset-0 overflow-hidden" style={{ zIndex: 0 }}>
      ...
      <motion.div style={{ y: y3 }} className="absolute inset-0 pointer-events-none">
        <motion.div
          ...
          className="absolute top-[22%]  right-[32%] w-3  h-3  rounded-full bg-[#10b981]/35 dark:bg-[#10b981]/30"
        />
        <motion.div
          animate={{ opacity: [0.4, 1, 0.4] }}
          transition={{ duration: 3.8, repeat: Infinity, ease: 'easeInOut', delay: 0.6 }}
          className="absolute top-[45%]  left-[22%]  w-2  h-2  rounded-full bg-black/25 dark:bg-white/20"
        />
        <motion.div
          animate={{ opacity: [0.4, 1, 0.4] }}
          transition={{ duration: 4.2, repeat: Infinity, ease: 'easeInOut', delay: 1.2 }}
          className="absolute top-[65%]  right-[12%] w-4  h-4  rounded-full border border-[#10b981]/25 dark:border-[#10b981]/20"
        />
        <motion.div
          animate={{ opacity: [0.4, 1, 0.4] }}
          transition={{ duration: 3.5, repeat: Infinity, ease: 'easeInOut', delay: 1.8 }}
          className="absolute top-[80%]  left-[40%]  w-3  h-3  rotate-45 border border-black/20 dark:border-white/15"
        />
        <motion.div
          animate={{ opacity: [0.4, 1, 0.4] }}
          transition={{ duration: 4.5, repeat: Infinity, ease: 'easeInOut', delay: 2.4 }}
          className="absolute top-[10%]  left-[38%]  w-2  h-2  rounded-full bg-black/20 dark:bg-white/15"
        />
      </motion.div>
    </div>
  )
