  ...
  return (
    <motion.header
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled || mobileOpen
          ? 'bg-[#080810]/95 backdrop-blur-md border-b border-white/5'
          : ''
      }`}
    >
      {/* Barra principal */}
      <div className={`max-w-6xl mx-auto px-6 flex items-center justify-between transition-all duration-500 ${scrolled ? 'py-4' : 'py-7'}`}>
        ...
        <nav className="flex items-center gap-8">
          ...
          <button
            ...
            className="text-sm font-medium px-4 py-1.5 rounded-full border border-white/10 text-gray-300 hover:border-[#c8ff00]/50 hover:text-[#c8ff00] transition-colors hidden md:block"
          >
            Contacto
          </button>
            ...
            <div className="max-w-6xl mx-auto px-6 pb-6 pt-2 flex flex-col gap-1">
              {links.map(({ label, hash }) => (
                <button
                  key={hash}
                  onClick={() => goToSection(hash)}
                  className="text-left text-gray-300 hover:text-white py-3 text-base font-medium border-b border-white/5 transition-colors"
                >
                  {label}
                </button>
              ))}
              <button
                onClick={() => goToSection('contact')}
                className="mt-4 text-sm font-medium px-5 py-2.5 rounded-full border border-white/10 text-gray-300 hover:border-[#c8ff00]/50 hover:text-[#c8ff00] transition-colors w-fit"
              >
                Contacto
              </button>
            </div>
